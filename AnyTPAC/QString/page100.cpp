    ...
    ui(new Ui::page100)
{
    ...
    myKeyboard = new alphanumpad(myValue, false, NULL, false);
}
...
void page100::on_atcmButton_String_clicked()
{
    // Set Current value to be edited
    myKeyboard->setQStringValue(ui->atcmButton_String->text());
    // Set alphanumpad prompt label
    myKeyboard->setPrompt("Enter Value / Text:");

    // Show alphanumpad object
    myKeyboard->showFullScreen();
    if (myKeyboard->exec() == QDialog::Accepted)  {
        // Retrieve input value
        QString newValue = myKeyboard->getQStringValue(); //Local QString
        newValue = newValue.trimmed();
        newValue.replace(QString(" "), QString("_"));

        Global_newValue = newValue; //Global_newValue is a global variable define in automation.cpp and automation.h

        // Update button text if changed
        if (newValue != ui->atcmButton_String->text())  {
            ui->atcmButton_String->setText(newValue);
            ui->label_String->setText(newValue);
        }
    }
