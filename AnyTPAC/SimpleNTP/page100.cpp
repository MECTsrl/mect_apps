...
char *page100::ntpServer(char *buffer, int size)
{
    if (size > ui->ntp_server->text().length()) {
        strncpy(buffer, ui->ntp_server->text().toLatin1().data(), size);
        return buffer;
    } else {
        return NULL;
    }
...
void page100::append(const char *text)
{
