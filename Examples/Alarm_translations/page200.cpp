...
void page200::on_atcmButton_ita_pressed()
{
    if(translator->load(QString(":/translations/languages_it.qm"))) {
        QSettings settings(CONFIG_FILE,QSettings::IniFormat);
        settings.setValue("SYSTEM/language","it");
        settings.sync();
        doWrite_Tipo_Lingua(0); //retentive value for alarm translation at boot
    }
    else
    {
...
void page200::on_atcmButton_eng_pressed()
{
    if(translator->load(QString(":/translations/languages_en.qm"))) {
        QSettings settings(CONFIG_FILE,QSettings::IniFormat);
        settings.setValue("SYSTEM/language","en");
        settings.sync();
        doWrite_Tipo_Lingua(1); //retentive value for alarm translation at boot
    }
    else
    {
      QMessageBox::information(this,trUtf8("Error"),trUtf8("Loading language"));
    }
    Translate_EN();
}
