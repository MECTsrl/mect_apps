...
#define ID_MQTT_Broker 200
extern int MQTT_Broker;	/* MQTT_Broker connected */
int doWrite_MQTT_Broker(int value);
int addWrite_MQTT_Broker(int value);
int getStatus_MQTT_Broker(void);

#define ID_MQTT_ActiveTopic 201
extern int MQTT_ActiveTopic;	/* MQTT Topic active */
int doWrite_MQTT_ActiveTopic(int value);
int addWrite_MQTT_ActiveTopic(int value);
int getStatus_MQTT_ActiveTopic(void);

#define ID_MQTT_Status 202
extern int16_t MQTT_Status;	/* Stato Connessione MQTT (significato simile ai Nodi Modbus) */
int doWrite_MQTT_Status(int16_t value);
int addWrite_MQTT_Status(int16_t value);
int getStatus_MQTT_Status(void);

#define ID_MQTT_mid 203
extern u_int32_t MQTT_mid;	/* MQTT Message Id of a sent message confirmed from Broker (QOS=2) */
int doWrite_MQTT_mid(u_int32_t value);
int addWrite_MQTT_mid(u_int32_t value);
int getStatus_MQTT_mid(void);

#define ID_MQTT_LastConnection 204
extern u_int32_t MQTT_LastConnection;	/* Uptime Seconds of last Connection */
int doWrite_MQTT_LastConnection(u_int32_t value);
int addWrite_MQTT_LastConnection(u_int32_t value);
int getStatus_MQTT_LastConnection(void);

#define ID_MQTT_LastDisconnection 205
extern u_int32_t MQTT_LastDisconnection;	/* Uptime Seconds of last Disconnection */
int doWrite_MQTT_LastDisconnection(u_int32_t value);
int addWrite_MQTT_LastDisconnection(u_int32_t value);
int getStatus_MQTT_LastDisconnection(void);
